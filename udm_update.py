...
"""
Command line tool to update UDM objects defined in a CSV file in UCS.
...
Version: 2025-08-08 (Fixes AttributeError, updates users with PasswordRecoveryEmailVerified and networkAccess)
"""
...
def main(ctx, udm_module: str, action: str, filename: str) -> None:
    ...
    """
    print('Running udm_update.py version 2025-08-08')
...
class UdmImport:
    ...
    def set_attrs(self, obj: Any, row: Dict[str, str]) -> None:
        for k, v in row.items():
            if k in self.obj_non_props:
                setattr(obj, k, v)
            else:
                setattr(obj.props, k, v)
        # Set additional attributes for modify action
        if self.action == 'modify':
            setattr(obj.props, 'PasswordRecoveryEmailVerified', 'TRUE')
            setattr(obj.props, 'networkAccess', '1')
